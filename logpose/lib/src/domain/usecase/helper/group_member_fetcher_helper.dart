...
final groupMemberFetcherHelperProvider =
    Provider<GroupMemberFetcherHelper>((ref) {
  final groupScheduleController = ref.read(groupScheduleUseCaseProvider);
  final userUseCase = ref.read(userUseCaseProvider);

  return GroupMemberFetcherHelper(
    groupScheduleController: groupScheduleController,
    userUseCase: userUseCase,
  );
});

class GroupMemberFetcherHelper {
  const GroupMemberFetcherHelper({
    required this.groupScheduleController,
    required this.userUseCase,
  });
  final GroupScheduleUseCase groupScheduleController;
  ...
  Future<String?> _fetchUserId(String scheduleId, String userId) async {
    try {
      return await GroupMemberScheduleRepository.fetchUserDocIdByTerm(
        scheduleId,
        userId,
      );
  ...
  Future<UserProfile> _fetchUserProfile(String userId) async {
    try {
      return await userUseCase.fetchUserProfile(userId);
    } on FirebaseException catch (e) {
